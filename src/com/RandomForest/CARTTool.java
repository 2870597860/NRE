...
public class CARTTool {
    // 类标号的值类型
    private final String CLIENT = "客户关系";
    private final String SUPPORT = "供应关系";
    private final String DEVLOP = "技术研发";
    private final String CON_MOTHER = "控股母公司";
    private final String CON_SON = "控股子公司";
    ...
    public double computeGini(String[][] remainData, String attrName,
            ...
            if (attrName.equals(attrNames[j])) {
                for (int i = 1; i < remainData.length; i++) {
                    ...
                    if ((beLongValue && remainData[i][j].equals(value))
                        ...
                        if (remainData[i][attrNames.length - 1].equals(CLIENT)) {
                            // 判断此行数据是否为正实例
                            ClientNum++;
                        } else if(remainData[i][attrNames.length - 1].equals(SUPPORT)){
                            SupportNum++;
                        }else if(remainData[i][attrNames.length - 1].equals(DEVLOP)){
                            DevlopNum++;
                        }else if(remainData[i][attrNames.length - 1].equals(CON_MOTHER)){
                            ConMoNum++;
                        }else{
                        	ConSonNum++;
                        }
        ...
        total = ClientNum + SupportNum + DevlopNum + ConMoNum + ConSonNum;
        double clientProbobly = (double) ClientNum / total;
        double supportProbobly = (double) SupportNum / total;
        double devProbobly = (double) DevlopNum / total;
        double conMoProbobly = (double) ConMoNum / total;
        double conSonProbobly = (double) ConSonNum / total;
        gini = 1 - clientProbobly * clientProbobly - supportProbobly * supportProbobly-
        		devProbobly*devProbobly-conMoProbobly*conMoProbobly-conSonProbobly*conSonProbobly;
    ...
    private void computeAlpha(TreeNode node) {
        ...
        for (TreeNode attrNode : leafNodes) {
            ...
            for (String s : dataIndex) {
                // 统计分类数据中的正负实例数
                if (data[Integer.parseInt(s)][attrNames.length - 1].equals(CLIENT)) {
                    num++;
                }
            }
            minNum += num;
